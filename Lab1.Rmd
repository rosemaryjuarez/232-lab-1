...
it seems like during the pumpkin season, october has the highest rate of sales for pumpkins, with sales slowing down as it reaches to december. However its difficult to distinguish which days are months.
...
pumpkins %>%
  ...
  geom_col(fill = "orangered1", alpha = 0.7) +
  ylab("Pumpkin Price")
...
from looking at the baked_pumpkins tibble, the total cities represented are 10, going from a list from 0-9
...
From the three that i explored, the variable with the highest correlation with price is the package of the pumpkins. This is likely because the price of a pumpkin is dependent of the weight and size of a pumpkin. Since the package of a pumpkin considers the weight and size of a pumpkin for a sale, there is likely high correlation.
...
The issue i see here with the model is that the linear regression does not fit the data correctly. We see here that the test points do not seem to fit the line in any way. The correlation seems to likely be low after seeing this.
